...
using static sln_TP.SV_Interlocking;
using sln_TP.Model;
...
namespace sln_TP
{
    public partial class frmDSVInterlockingConfig : Form
    {
        ...
        {
            long s = SV_Interlocking.sv_lockInfo(txtZSN.Text);
            if (s == 0)
            {
        ...
        private void updateMes(bool isok)
        {
            ...
            try
            {
                UUT_RESULT uut_result = new UUT_RESULT
                {
                    ...
                    BATCH_SERIAL_NUMBER = txtHSN.Text,
                    UUT_SERIAL_NUMBER = txtZSN.Text,
                    USER_LOGIN_NAME = utility.dSV.SW_User,
                    START_DATE_TIME = DateTime.Now,
                    UUT_STATUS = isok ? "Passed" : "Failed"
                };
                CONT_UUT_RESULT.InsertMesData(uut_result);
                logtitle = "上传mes成功" + (isok ? "OK,产品主SN码:" : "NG,产品主SN码:") + txtZSN.Text+ " 产品后壳SN码:" + txtHSN.Text;
                MessageBox.Show(logtitle);
            }
            catch (Exception exception)
            {
                logtitle = "上传mes失败," + exception.Message + (isok ? "OK,产品主SN码:" : "NG,产品主SN码:") + txtZSN.Text + " 产品后壳SN码:" + txtHSN.Text;
                MessageBox.Show(logtitle);
            }
            finally
            {
