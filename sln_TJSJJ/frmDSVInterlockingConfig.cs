...
using static sln_TJSJJ.SV_Interlocking;
    ...
    public partial class frmDSVInterlockingConfig : Form
    {
        ...
        private void updateMes(bool isok)
        {
            ...
            try
            {
                ...
                logtitle = "上传mes成功" + (isok ? "OK,产品后壳SN码:" : "NG,产品后壳SN码:") + txtSN.Text;
                MessageBox.Show(logtitle);
            }
            catch (Exception exception)
            {
                logtitle = "上传mes失败," + exception.Message + (isok ? "OK,产品后壳SN码:" : "NG,产品后壳SN码:") + txtSN.Text;
                MessageBox.Show(logtitle);
            }
            finally
            {
