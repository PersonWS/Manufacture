...
namespace ScrewMachineManagementSystem.CenterControl
{
    ...
    public class BusinessMain
    {
        ...
        public bool BusinessStart()
        {
            if (_isBusinessStart)
            {
                MessageOutputMethod("Business already  Start " ,1);
                return true;
            }
            ////检查实例
            //if (_plcConnect==null)
            ...
            _plc_monitor.pointValueChanged += PointValueChanged;
            MessageOutputMethod("正在启动PLC点位监控服务...",0);
        ...
        private void PointValueChanged(PLC_Point point)
        {
            ...
            try
            {
                switch (point.VarName)
                {
                    case "SN码请求"://SN码请求 为1 时，置位所有需要写入的数据
                        ...
                        {
                            MessageOutput_BusinessMethod(string.Format("PLC:{0} 发出SN码请求", PLC_Connect.PlcEntity.IP), 0);
                            SN_CodeRequest(point);
                        }
                        else
                        { MessageOutput_BusinessMethod(string.Format("SN:{0} SN码请求信号 复位  0", _SN_code), 0); }
                        break;
                    case "开始加工请求"://开始加工请求  , 需要向外部请求该SN码的过站信息，是否为本工序上一站
                        if ((bool)point.value == true)
                        {
                            MessageOutput_BusinessMethod(string.Format("SN:{0} 发出  开始加工请求", _SN_code), 0);
                            ManufactureRequest(point);
                        }
                        else
                        { MessageOutput_BusinessMethod(string.Format("SN:{0} 开始加工请求信号 复位  0", _SN_code), 0); }
                        break;
                    case "结果OK"://结果OK 为1 时，置位所有需要写入的数据
                        if ((bool)point.value == true)
                        {
                            MessageOutput_BusinessMethod(string.Format("SN:{0} 加工完成，加工结果：OK", _SN_code), 0);
                            ManufactureResultOutput();
                        }
                        else
                        { MessageOutput_BusinessMethod(string.Format("SN:{0} 结果OK信号  复位  0", _SN_code), 0); }
                        break;
                    case "结果NG"://SN码请求 为1 时，置位所有需要写入的数据
                        if ((bool)point.value == true)
                        {
                            MessageOutput_BusinessMethod(string.Format("SN:{0} 加工完成，加工结果：NG", _SN_code), 0);
                            ManufactureResultOutput();
                        }
                        else
                        { MessageOutput_BusinessMethod(string.Format("SN:{0} 结果NG信号  复位  0", _SN_code), 0); }
                        break;
                    case "表格清空"://表格清空 为1 时清螺丝机电批数据
                        if ((bool)point.value == true)
                        {
                            MessageOutput_BusinessMethod("表格清空信号=1  ，准备清空电批数据", 0);
                        ...
                        else//置为 0时将 表格已清空信号 复位为0
                        {
                            ...
                            MessageOutput_BusinessMethod("表格清空,电批表格数据已清空,信号复位", 0);
                        }
                        break;
                    case "SN码":
            ...
            {
                MessageOutputMethod(string.Format("SN:{0}  pointName:{1}  执行时发生错误 ，ex={2}", _SN_code, point.VarName, ex.ToString()), 1);
            }
        ...
        {
            MessageOutput_BusinessMethod("收到表格清空请求,清理上次的遗留状态，清理开始...",0);
            ...
            if (Need_ClearScrewData != null)
            {
                if (Need_ClearScrewData())
                {
                    ...
                    {
                        MessageOutput_BusinessMethod("电批数据已清空", 0);
                        return true;
                    }
                    else
                    {
                        MessageOutput_BusinessMethod("电批数据显示反馈结果设置失败，准备重新发起电批数据清理，表格清空  需为 1", 1);
                ...
                {
                    MessageOutput_BusinessMethod("电批数据显示清理失败，准备重新发起电批数据清理，表格清空  需为 1",1);
                    BusinessNeedPlcPoint.Dic_gatherPLC_Point["表格清空"].value = false;
                    return false;
                }
            }
            ...
            {
                MessageOutput_BusinessMethod("未执行清理电批数据任务，请检查【Need_ClearScrewData】事件是否已订阅",1);
                return false;
            }
        }
        ...
        private void ResetScrewDataClearSignal()
        {
            ...
            if (WriteData_RetryLimit5(BusinessNeedPlcPoint.Dic_gatherPLC_Point["表格已清空"]))//写入成功 
            {
                MessageOutput_BusinessMethod("[表格已清空] 信号复位为  0  成功", 0);
            }
            else
            {
                MessageOutput_BusinessMethod("[表格已清空] 信号复位为  0  失败 ，准备重新写入",1);
                BusinessNeedPlcPoint.Dic_gatherPLC_Point["表格清空"].value = false;
            }
        ...
        private void ManufactureResultOutput()
        {
            ...
            {
                MessageOutput_BusinessMethod(string.Format("SN:{0} OK NG 同时输出", _SN_code),1);
            }
            else
            {
                if (string.IsNullOrEmpty(_SN_code))
                {
                    MessageOutput_BusinessMethod("SN码未输入时检测到 OK / NG 信号输出，不处理",1);
                    return;
                }
                if ((bool)BusinessNeedPlcPoint.Dic_gatherPLC_Point["允许加工请求"].value != true)
                {
                    MessageOutput_BusinessMethod("不允许加工时， OK / NG 信号输出，不处理",1);
                    return;
                }
                ...
                { MessageOutput_BusinessMethod("向MES传递并保存信息失败，请检查【SaveInformationToMES_Result_Request】事件是否已订阅",1); }
        ...
        public bool ManufactureResultWriteToPLC(string SN, bool result, bool isForceWrite = false)
        {
            ...
            {
                MessageOutput_BusinessMethod("SN码校验失败， PLC 加工结果收到 未写入PLC",1);
                return false;
            }
            if (result)
            {
                ...
                {
                    MessageOutput_BusinessMethod("PLC 加工结果收到 写入成功", 0);
                    return true;
                }
                else
                {
                    MessageOutput_BusinessMethod("PLC 加工结果收到 写入失败", 1);
                    return false;
                }

            }
            else
            { MessageOutput_BusinessMethod("加工结果记录失败，不向PLC写入记录完成信号", 1); return false; }
        ...
        {
            MessageOutput_BusinessMethod("SN码写入前,清理上次的遗留状态，清理开始...", 0);
            ClearPointData_Reset0();
            ...
            if (Need_SN_Request != null)//向外部请求SN码
            {
                ...
                {
                    MessageOutput_BusinessMethod(string.Format("已获得SN:{0} ,本次传入的SN:{1}不生效", _SN_code, snCode), 1);
                    return;
                }
                ...
                else
                {
                    ...
                    {
                        MessageOutput_BusinessMethod("SN写入失败，准备重新发起SN申请，SN码请求  需为 1", 1);
                        point.value = false;
                    }
            ...
            { MessageOutput_BusinessMethod("外部未传入SN，请检查【Need_SN_Request】事件是否已订阅", 1); }
        ...
        public bool ClearPointData_Reset0()
        {
           ...
           bool a=  WriteData_RetryLimit5(BusinessNeedPlcPoint.Dic_gatherPLC_Point["表格已清空"]);
            MessageOutput_BusinessMethod("表格已清空 已清除", 0);
            ...
            MessageOutput_BusinessMethod("PLC SN码已清除", 0);
            ...
            MessageOutput_BusinessMethod("PLC 允许加工请求 已清除", 0);
            ...
            MessageOutput_BusinessMethod("PLC 禁止加工请求 已清除", 0);
            ...
            MessageOutput_BusinessMethod("PLC 互锁结果 已清除", 0);
            ...
            MessageOutput_BusinessMethod("PLC 加工结果收到 已清除", 0);
        ...
        public bool WriteSN_ToPLC(string snCode)
        {
            ...
            {
                MessageOutput_BusinessMethod(string.Format("SN写入成功，写入的SN;{0}", snCode), 0);
                _SN_code = snCode;
                return true;
            }
            else
            {
                MessageOutput_BusinessMethod(string.Format("SN写入失败，需要写入的SN;{0}，请重新执行SN_CodeRequest", snCode), 1);
                return false;
            }
        }
        ...
        public void ManufactureRequest(PLC_Point point)
        {
            ...
            if (string.IsNullOrEmpty(_SN_code))
            {
                MessageOutput_BusinessMethod("收到加工请求，但SN码未写入，不处理", 1);
                point.value = false;
            ...
            if (Need_lastProcessName_Request != null)
            {
                ...
                {
                    MessageOutput_BusinessMethod(string.Format("SN:{0} ,开始加工已申请，本次申请不生效", _SN_code), 1);
                    return;
                }
                if (string.IsNullOrEmpty(lastProcessIn))
                {
                    MessageOutput_BusinessMethod("外部传入的上一工序为空，准备重新发起工序校验申请，是否加工  需为 1", 1);
                    System.Threading.Thread.Sleep(1000);
                    point.value = false;
                    return;
                }
                ...
                {
                    MessageOutput_BusinessMethod("写入工序校验结果失败，准备重新发起工序校验申请，是否加工  需为 1", 1);
                    System.Threading.Thread.Sleep(1000);
                    point.value = false;
                }
                else
            ...
            { MessageOutput_BusinessMethod(string.Format("SN:{0}收到加工请求，但外部未传入上一道工序名称，请检查【Need_lastProcessName_Request】事件是否已订阅", _SN_code), 1); }
        }
        /// <summary>
        ...
        public bool LastProcessNameCheck(string lastProcessName, bool isForceWrite = false)
        {
            ...
            {
                MessageOutput_BusinessMethod(string.Format("SN:{0}  上一工序符合要求，准备开始写入加工指令", _SN_code), 0);
                BusinessNeedPlcPoint.Dic_gatherPLC_Point["禁止加工请求"].value = false;
                ...
                if (a && b && c)
                {
                    MessageOutput_BusinessMethod(string.Format("SN:{0}  PLC 允许加工请求 写入成功", _SN_code), 0);
                    //_manufacturePermission = true;
                    return true;
                }
                else
                {
                    MessageOutput_BusinessMethod("PLC 允许加工请求 写入失败", 1);
                    return false;
                }
            }
            else
            {
                MessageOutput_BusinessMethod(string.Format("产品工序校验失败！  SN:{0}  需要的上道工序为{1} ,实际上道工序为：{2}", _SN_code, _lastProcessName, lastProcessName), 0);
                ...
                if (b && c)
                {
                    MessageOutput_BusinessMethod(string.Format("SN:{0}  PLC 禁止加工请求 写入成功", _SN_code), 0);
                    return true;

                }
                else
                {
                    MessageOutput_BusinessMethod(string.Format("SN:{0}  PLC 禁止加工请求 写入失败", _SN_code), 1);
                }
                MessageOutput_BusinessMethod(string.Format("SN:{0}  上一工序不符合要求，传入的上一工序为：{0}，需要的上一工序为：{1}，请检查", _SN_code, lastProcessName, _lastProcessName), 1);
                return false;
        ...
        private void MessageOutput_BusinessMethod(string s, int level)
        {
