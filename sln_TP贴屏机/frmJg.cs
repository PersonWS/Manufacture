...
namespace sln_TP
{
    public partial class frmJg : Form
    {
        ...
        {
            if((DateTime.Now - Corecurrent.GetCorecurrent().dtTime).Seconds<10)
            {
                this.lbltime.Text = (10-(DateTime.Now - Corecurrent.GetCorecurrent().dtTime).Seconds).ToString() + "秒后自动关闭";
            }
            else
            {
                this.Hide();
