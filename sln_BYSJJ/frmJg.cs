...
namespace sln_BYSJJ
{
    public partial class frmJg : Form
    {
        ...
        {
            if((DateTime.Now - Corecurrent.GetCorecurrent().BY_dtTime).Seconds<10)
            {
                this.lbltime.Text = (10-(DateTime.Now - Corecurrent.GetCorecurrent().BY_dtTime).Seconds).ToString() + "秒后自动关闭";
            }
            else
            {
                this.Hide();
